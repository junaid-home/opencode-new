  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      if (isImage || isPdfAttachment(mime)) {
        ...
        // The attachment makes the image visible to the model, but the user only
        // sees it when the reply references the local path as markdown. Spell
        // out the exact line so the model echoes it instead of describing the
        // image in prose.
        const alt = path.basename(filepath).replace(/\.[^.]+$/, "")
        const output = [
          `Image read successfully: ${filepath}`,
          ``,
          `Show this image to the user in your reply. Reference the absolute path with standard markdown, exactly as shown (the client renders local image paths):`,
          `![${alt}](${filepath})`,
        ].join("\n")
        return {
          title,
      ...
      if (isVideoAttachment(mime)) {
        const msg = `Video read successfully: ${filepath}`
        return {
          title,
          output: msg,
          metadata: {
