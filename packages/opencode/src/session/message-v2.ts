    ...
    if (msg.info.role === "user") {
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "file" && part.mime !== "text/plain" && part.mime !== "application/x-directory") {
          ...
          } else if (isVideoAttachment(part.mime)) {
            if (model.capabilities.input.video && !options?.stripMedia) {
              userMessage.parts.push({
                type: "file",
                url: part.url,
                mediaType: part.mime,
              ...
              // Models that cannot consume video only learn the attachment
              // exists; the preview stays visible to the user.
              userMessage.parts.push({
                type: "text",
                text: `[Attached ${part.mime}: ${part.filename ?? "file"}] (video preview shown to the user; not viewable by the model)`,
              })
            }
          } else {
            userMessage.parts.push({
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            ...
            // Videos are display-only: models cannot consume them, so strip them
            // from the provider payload entirely (they stay in the stored part for
            // the client to render).
            const finalAttachments = attachments.filter(
              (a) => (!isMedia(a.mime) || supportsMediaInToolResult(a)) && !isVideoAttachment(a.mime),
            )
