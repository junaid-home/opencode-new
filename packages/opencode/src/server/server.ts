...
function serverLayer(opts: { port: number; hostname: string }) {
  const { decodeDirectory, getThumbnailFilePath } = require("@/preview/thumbnail") as typeof import("@/preview/thumbnail")
  const { readFileSync } = require("fs") as typeof import("fs")

  const server = createServer((req, res) => {
    const url = new URL(req.url || "/", `http://${req.headers.host || "localhost"}`)
    if (url.pathname.startsWith("/preview/thumbnail/")) {
      const encodedDir = url.pathname.split("/preview/thumbnail/")[1]
      if (!encodedDir) { res.writeHead(400, { "Content-Type": "application/json" }); res.end('{"error":"Invalid path"}'); return }
      const directory = decodeDirectory(encodedDir)
      if (!directory) { res.writeHead(400, { "Content-Type": "application/json" }); res.end('{"error":"Invalid directory"}'); return }
      const thumbnailPath = getThumbnailFilePath(directory)
      if (!thumbnailPath) { res.writeHead(404, { "Content-Type": "application/json" }); res.end('{"error":"Thumbnail not found"}'); return }
      try {
        const imageBuffer = readFileSync(thumbnailPath)
        res.writeHead(200, { "Content-Type": "image/png", "Cache-Control": "public, max-age=300" })
        res.end(imageBuffer)
      } catch {
        res.writeHead(500, { "Content-Type": "application/json" }); res.end('{"error":"Failed to read"}')
      }
      return
    }
    // For all other requests, NodeHttpServer.layer's listener handles them
  })
