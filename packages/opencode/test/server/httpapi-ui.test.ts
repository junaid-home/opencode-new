...
import { previewProxyResponse } from "../../src/server/preview-proxy"
...
describe("HttpApi preview proxy", () => {
  it.live("proxies preview dev server assets and prevents caching", () =>
    Effect.gen(function* () {
      let proxiedUrl: string | undefined
      const client = HttpClient.make((request) => {
        ...
        return Effect.succeed(
          HttpClientResponse.fromWeb(
            request,
            new Response("console.log('bundle')", { headers: { "content-type": "text/javascript" } }),
          ),
        )
      })

      const response = yield* previewProxyResponse(
        client,
        HttpServerRequest.fromWeb(new Request("http://localhost/global/preview/proxy/60244/static/js/bundle.js")),
        { port: 60244, path: "/static/js/bundle.js" },
      ).pipe(Effect.map(HttpServerResponse.toWeb))

      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("http://localhost:60244/static/js/bundle.js")
      expect(response.headers.get("cache-control")).toBe("no-store")
      expect(yield* responseText(response)).toBe("console.log('bundle')")
    }),
  )

  it.live("rewrites preview HTML asset URLs onto the proxy path", () =>
    Effect.gen(function* () {
      const client = HttpClient.make((request) =>
        Effect.succeed(
          HttpClientResponse.fromWeb(
            request,
            new Response('<html><head><link rel="icon" href="/favicon.ico"></head><body>app</body></html>', {
      ...
      const response = yield* previewProxyResponse(
        client,
        HttpServerRequest.fromWeb(new Request("http://localhost/global/preview/proxy/60244")),
        { port: 60244, path: "/" },
      ).pipe(Effect.map(HttpServerResponse.toWeb))
      ...
      expect(body).toContain('<base href="/global/preview/proxy/60244/">')
      expect(body).toContain('href="/global/preview/proxy/60244/favicon.ico"')
    }),
  )

  // The fetch-based HttpClient decompresses upstream gzip transparently but
  // leaves `content-encoding: gzip` behind; the buffered body is already plain
  // text, so the proxy must not try to gunzip it again. Upstream framing
  // headers are also stripped so the re-framed response stays valid.
  it.live("passes through already-decompressed bodies despite a stale content-encoding header", () =>
    Effect.gen(function* () {
      const client = HttpClient.make((request) =>
        Effect.succeed(
          HttpClientResponse.fromWeb(
            request,
            new Response("console.log('bundle')", {
              headers: {
