...
describe("tool.read video files", () => {
  it.live("reads video files and reports success without embedding bytes", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      const bytes = Buffer.from([0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x6d, 0x70, 0x34, 0x32])
      yield* put(path.join(dir, "clip.mp4"), bytes)
  ...
  it.live("video files are not rejected as binary", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      yield* put(path.join(dir, "clip.webm"), "webm container placeholder")

      const result = yield* exec(dir, { filePath: path.join(dir, "clip.webm") })
      expect(result.output).toContain("Video read successfully")
    }),
  )
})
