...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("converts user video file parts to a text placeholder", async () => {
    const messageID = "m-user"
        ...
        parts: [
          {
            ...basePart(messageID, "p1"),
            type: "text",
            text: "can you use the color theme from the video?",
          },
          {
            ...basePart(messageID, "p2"),
            type: "file",
    ...
    expect(await MessageV2.toModelMessages(input, model)).toStrictEqual([
      {
        role: "user",
        content: [
          { type: "text", text: "can you use the color theme from the video?" },
          {
            type: "text",
            text: "[Attached video/mp4: clip.mp4] (video preview shown to the user; not viewable by the model)",
          },
        ],
      },
    ])
  ...
  test("passes user video file parts through for video-capable models", async () => {
    const messageID = "m-user"

    const videoModel: Provider.Model = {
      ...model,
      capabilities: {
        ...model.capabilities,
        input: {
    ...
    expect(await MessageV2.toModelMessages(input, videoModel)).toStrictEqual([
      {
        role: "user",
        content: [
          { type: "text", text: "can you use the color theme from the video?" },
