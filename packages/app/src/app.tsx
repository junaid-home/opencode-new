...
function AppLayout(props: ParentProps<{ serverScoped?: JSX.Element }>) {
  return <NewAppLayout serverScoped={props.serverScoped}>{props.children}</NewAppLayout>
}
...
function AuthGuard(props: ParentProps) {
  ...
  const shouldRender = () => !auth.loading() && (auth.isAuthenticated() || isPublicPath())
...
function Routes(props: { serverScoped?: JSX.Element }) {
  return (
    <>
      ...
      <Route path="/signup-secret-route" component={SignupPage} />
