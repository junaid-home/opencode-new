...
export type FileTreeV2LiveRoot = {
  name: string
  zip: string
}
...
export function flattenLiveFileTreeV2(
  ...
  root?: FileTreeV2LiveRoot,
) {
  const rows: FileTreeV2Row[] = []
  const stack: FileTreeV2Row[] = []
  const rootExpanded = !root || expanded("")
  if (rootExpanded) {
    const level = root ? 1 : 0
    if (root) {
      stack.push({
        node: { name: root.zip, path: root.zip, absolute: root.zip, type: "file", ignored: false, originalPath: root.zip },
        level,
      })
    }
    const base = children("")
      .toReversed()
      .map((node) => ({ node: toLiveNode(node), level }))
    stack.push(...base)
  }
  if (root) {
    stack.push({ node: { name: root.name, path: "", absolute: "", type: "directory", ignored: false, originalPath: "" }, level: 0 })
  }
  ...
  while (stack.length > 0) {
    ...
    rows.push(row)
    if (row.node.type !== "directory" || !expanded(row.node.path)) continue
    if (root && row.node.path === "") continue
