...
describe("flattenLiveFileTreeV2", () => {
  ...
  test("wraps children under a project root with a trailing zip file", () => {
    const nodes: Record<string, FileNode[]> = {
      "": [
        { name: "src", path: "src", absolute: "/repo/src", type: "directory", ignored: false },
        { name: "README.md", path: "README.md", absolute: "/repo/README.md", type: "file", ignored: false },
      ],
      src: [{ name: "a.ts", path: "src/a.ts", absolute: "/repo/src/a.ts", type: "file", ignored: false }],
    }
    ...
    expect(
      flattenLiveFileTreeV2(
        (path) => nodes[path] ?? [],
        (path) => path === "src" || path === "",
        { name: "xtreme", zip: "xtreme.zip" },
      ).map((row) => [row.node.path, row.node.type, row.level]),
    ).toEqual([
      ["", "directory", 0],
      ["src", "directory", 1],
      ["src/a.ts", "file", 2],
  ...
  test("shows only the project root and zip when the project is empty", () => {
    const rows = flattenLiveFileTreeV2(() => [], () => true, { name: "empty", zip: "empty.zip" })
    expect(rows.map((row) => row.node.path)).toEqual(["", "empty.zip"])
  })

  test("collapsing the project root hides its children", () => {
    const nodes: Record<string, FileNode[]> = {
      "": [{ name: "a.ts", path: "a.ts", absolute: "/repo/a.ts", type: "file", ignored: false }],
    }
    const rows = flattenLiveFileTreeV2((path) => nodes[path] ?? [], () => false, { name: "xtreme", zip: "xtreme.zip" })
