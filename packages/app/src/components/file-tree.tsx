...
function FileContextMenu(props: {
  ...
  const [renameValue, setRenameValue] = createSignal(props.node.name)
  let renameInput: HTMLInputElement | undefined

  const handleDownload = async () => {
    try {
      await file.load(props.node.path, { force: true })
      const state = file.get(props.node.path)
      if (!state?.content) return
      let blob: Blob
      if (state.content.type === "binary") {
        const binary = Uint8Array.from(atob(state.content.content), (c) => c.charCodeAt(0))
        blob = new Blob([binary], { type: state.content.mimeType ?? "application/octet-stream" })
      } else {
        blob = new Blob([state.content.content], { type: "application/octet-stream" })
      }
  ...
  return (
    <div
      class="flex items-center group/menu w-full min-w-0 h-6 rounded-md px-1.5 py-0 hover:bg-surface-raised-base-hover active:bg-surface-base-active transition-colors cursor-pointer"
      style={`padding-left: ${Math.max(0, 8 + props.level * 12 - 24)}px`}
    >
      <button
        type="button"
        class="flex items-center gap-x-1.5 flex-1 min-w-0 text-left"
        onClick={() => !renaming() && props.onFileClick?.(props.node)}
      >
        ...
        <Switch>
          ...
          <Match when={!!kind() && !props.node.ignored}>
            <FileIcon
              node={props.node}
              class="size-4 filetree-icon filetree-icon--mono"
          ...
          <Match when={!props.node.ignored}>
            <span class="filetree-iconpair size-4">
              <FileIcon
                node={props.node}
                class="size-4 filetree-icon filetree-icon--color opacity-0 group-hover/menu:opacity-100"
              />
              <FileIcon
                node={props.node}
                class="size-4 filetree-icon filetree-icon--mono group-hover/menu:opacity-0"
                mono
              />
            </span>
          </Match>
          ...
          fallback={
            <span
              classList={{
                ...
                "text-text-weak": !props.node.ignored && !kind(),
              }}
              style={kind() ? kindTextColor(kind()!) : undefined}
            >
              {props.node.name}
            </span>
          }
        >
          <input
            ...
            class="flex-1 min-w-0 text-12-medium bg-surface-base border border-border-base rounded px-1 py-0 outline-none focus:border-primary-base"
      ...
      <DropdownMenu gutter={4} placement="bottom-end">
        <DropdownMenu.Trigger
          as="button"
          type="button"
          class="shrink-0 size-5 flex items-center justify-center rounded opacity-0 group-hover/menu:opacity-100 hover:bg-surface-base-active transition-opacity cursor-pointer"
          onClick={(e: MouseEvent) => e.stopPropagation()}
        >
          <Icon name="dot-grid" size="small" class="text-icon-weak" />
        </DropdownMenu.Trigger>
        <DropdownMenu.Portal>
          <DropdownMenu.Content>
            <DropdownMenu.Item onSelect={() => props.onFileClick?.(props.node)}>
              <DropdownMenu.ItemLabel>Open</DropdownMenu.ItemLabel>
            </DropdownMenu.Item>
            <DropdownMenu.Item onSelect={handleDownload}>
              <DropdownMenu.ItemLabel>Download</DropdownMenu.ItemLabel>
            </DropdownMenu.Item>
...
function ZipRow(props: { node: FileNode; onDownloadProject?: (name: string) => void }) {
  return (
    <div
      class="flex items-center group/menu w-full min-w-0 h-6 rounded-md px-1.5 py-0 cursor-default"
      style={`padding-left: ${Math.max(0, 8 - 24)}px`}
    >
      <div class="flex items-center gap-x-1.5 flex-1 min-w-0 text-left">
        <div class="w-4 shrink-0" />
        <span class="filetree-iconpair size-4">
          <FileIcon
            node={props.node}
        ...
        <span class="flex-1 min-w-0 text-12-medium whitespace-nowrap truncate text-text-weak">{props.node.name}</span>
      </div>
      <DropdownMenu gutter={4} placement="bottom-end">
        <DropdownMenu.Trigger
          as="button"
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content>
            <DropdownMenu.Item onSelect={() => props.onDownloadProject?.(props.node.name)}>
              <DropdownMenu.ItemLabel>Download</DropdownMenu.ItemLabel>
            </DropdownMenu.Item>
          </DropdownMenu.Content>
        </DropdownMenu.Portal>
...
export default function FileTree(props: {
  ...
  kinds?: ReadonlyMap<string, Kind>
  draggable?: boolean
  root?: { name: string; zip: string }
  onFileClick?: (file: FileNode) => void
  onFileDoubleClick?: (file: FileNode) => void
  onRename?: (oldPath: string, newPath: string) => void
  onDelete?: (path: string) => void
  onDownloadProject?: (name: string) => void
  ...
  const root = createMemo(() => (props.root && level === 0 ? props.root : undefined))
  const rootExpanded = () => file.tree.state(props.path)?.expanded ?? true
  ...
  return (
    <div data-component="filetree" class={`flex flex-col gap-0.5 ${props.class ?? ""}`}>
      <Show when={root()} fallback={children()}>
        {(value) => (
          <>
            <Collapsible
              variant="ghost"
              ...
              forceMount={false}
              open={rootExpanded()}
              onOpenChange={(open) => (open ? file.tree.expand(props.path) : file.tree.collapse(props.path))}
            >
              <Collapsible.Trigger>
                <FileTreeNode
                  node={{
