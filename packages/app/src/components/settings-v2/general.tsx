...
export const SettingsGeneralV2: Component<{
  ...
  const onPinchZoomChange = (checked: boolean) => {
    settings.general.setPinchZoom(checked)
    const update = platform.setPinchZoomEnabled?.(checked)
    if (!update) return
    void update.catch(() => settings.general.setPinchZoom(!checked))
  }
      ...
      <SettingsListV2>
        ...
        >
          <div data-action="settings-pinch-zoom">
            <Switch checked={settings.general.pinchZoom()} onChange={onPinchZoomChange} />
          </div>
        </SettingsRowV2>
      </SettingsListV2>
    </div>
