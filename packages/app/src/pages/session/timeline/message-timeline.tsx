...
type FramedTimelineRow = Exclude<TimelineRow.TimelineRow, { _tag: "TurnGap" } | { _tag: "PendingAnswer" }>
type TimelineRowByTag<T extends TimelineRow.TimelineRow["_tag"]> = Extract<TimelineRow.TimelineRow, { _tag: T }>

const timelineFallbackItemSize = 60
const timelineCacheSnapshotLimit = 1000
...
function TimelineThinkingRow(props: {
  ...
  return (
    <div data-slot="session-turn-thinking">
      <TextShimmer
        text={
          props.awaitingAnswer
            ? language.t("session.question.pendingAnswer")
...
export function MessageTimeline(props: {
  ...
  const [progressTimeoutDone, setProgressTimeoutDone] = createSignal(true)
  const working = createMemo(() => sessionStatus().type !== "idle")
  const workingStatus = createMemo<"hidden" | "showing" | "hiding">((prev) => {
    if (working()) return "showing"
    if (prev === "showing" || !progressTimeoutDone()) return "hiding"
    return "hidden"
  })
  createEffect(() => {
    if (workingStatus() !== "hiding") return
    setProgressTimeoutDone(false)
    const timer = setTimeout(() => setProgressTimeoutDone(true), 260)
  ...
  let pruneFrame: number | undefined
  onMount(() => {
  ...
  createEffect(() => {
    ...
    if (measuredSessionKey !== key) {
      ...
      virtualizer.measure()
    } else if (pruneFrame === undefined) {
      pruneFrame = requestAnimationFrame(() => {
        pruneFrame = undefined
        virtualizer.measureElement(null)
        const live = timelineRowByKey()
        for (const itemKey of virtualizer.itemSizeCache.keys()) {
          if (typeof itemKey === "string" && !live.has(itemKey)) virtualizer.itemSizeCache.delete(itemKey)
        }
      })
    }
    maybeAnchorBottom()
  ...
  onCleanup(() => {
    ...
    timelineCache.set(ownerSessionKey, {
      measurements: virtualizer.takeSnapshot().slice(-timelineCacheSnapshotLimit),
      toolOpen: { ...toolOpen },
    })
    while (timelineCache.size > 16) timelineCache.delete(timelineCache.keys().next().value!)
    if (resizePinFrame !== undefined) cancelAnimationFrame(resizePinFrame)
    if (overscanFrame !== undefined) cancelAnimationFrame(overscanFrame)
    if (pruneFrame !== undefined) cancelAnimationFrame(pruneFrame)
  ...
  const renderTimelineRow = (row: Accessor<TimelineRow.TimelineRow>, onSizeChange?: () => void) => {
    switch (row()._tag) {
      ...
      case "PendingAnswer":
        return (
          <div
            data-timeline-row="PendingAnswer"
            data-slot="session-turn-message-container"
            classList={{
              "w-full px-4 md:px-5": true,
              "md:max-w-200 2xl:max-w-[1000px]": props.centered,
              "md:mx-auto": props.centered,
            }}
          >
            <TimelineThinkingRow reasoningHeading={undefined} showReasoningSummaries awaitingAnswer />
          </div>
        )
      case "Retry": {
                      ...
                      fallback={
                        <>
                          ...
                          <h1
                            ...
                            classList={{
                              "truncate text-14-medium leading-4 text-v2-text-text-base": true,
                  ...
                  >
                    <div
                      ...
                      onClick={async () => {
                        ...
                        const newSession = await serverSDK().client.session.create({ directory })
                        const id = newSession.data?.id
                        if (id) navigate(legacySessionHref(directory, id))
                      }}
                    >
                      <Icon name="plus" class="cursor-pointer" size="small" />
                    </div>
                    <SessionContextUsage
                      placement="bottom"
