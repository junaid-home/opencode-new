...
export default function Page() {
  ...
  const agentModelName = createMemo(() => {
    const session = currentSession()
    if (!session?.model) return session?.agent
  ...
  const desktopSessionResizeOpen = createMemo(() => {
    if (newSessionDesign()) return desktopV2ReviewOpen() || desktopTerminalOpen()
    return desktopReviewOpen()
  })
  const [previewReady, setPreviewReady] = createSignal(false)
  const [activeSidePanelTab, setActiveSidePanelTab] = createSignal("")
  ...
  // padding and the left sidebar (rail + sessions panel), since the panel row
  // sits inside <main class="flex-1"> after those items. Only the flex gap
  // between the panels remains to subtract.
  const sessionPanelGap = createMemo(() => (settings.general.newLayoutDesigns() ? 8 : 0))
  const sessionPanelAvailable = createMemo(() => {
    const width = panelRowWidth()
    if (width === undefined) return undefined
    return sessionPanelAvailableWidth(width, sessionPanelGap())
  })
  ...
  createEffect(() => {
    ...
    layout.session.resize(chatPanelDefaultWidth(available))
  })
  ...
  const sessionPanelMin = createMemo(() => {
    const available = sessionPanelAvailable()
    if (available === undefined) return SESSION_PANEL_WIDTH_MIN
    return chatPanelWidthMin(available)
  })
  ...
  const sessionPanelCollapsed = layout.session.collapsed
  const setSessionPanelCollapsed = layout.session.setCollapsed
  const [previewNonce, setPreviewNonce] = createSignal(0)

  const sessionPanelWidth = createMemo(() => {
    if (sessionPanelCollapsed()) return "0px"
  ...
  const rootFiles = createMemo(() => projectRootFiles(file.tree.children("")))
  const projectHasFiles = createMemo(() => {
    if (!file.tree.state("")?.loaded) return false
    return rootFiles().length > 0
  })

  const [initialOpen, setInitialOpen] = createSignal(false)
  const [panelStateDecided, setPanelStateDecided] = createSignal(false)
  const decidingPanelState = createMemo(() => initialOpen() && !panelStateDecided())
  createEffect(() => {
    if (initialOpen()) return
    if (!sdk().directory || sync().status === "loading") return
    setInitialOpen(true)
    void file.tree.refresh("")
  })

  createEffect(() => {
    if (!initialOpen() || panelStateDecided()) return
    const root = file.tree.state("")
    if (!root?.loaded) {
      if (!root?.error) return
      setPanelStateDecided(true)
      return
    }
    setPanelStateDecided(true)
    if (projectHasFiles()) {
    ...
    } else if (!composer.questionRequest()) {
  ...
  createEffect(
    on(
      () => composer.questionRequest()?.id,
      (id) => {
        if (!id) return
        if (!view().reviewPanel.opened()) view().reviewPanel.open()
        if (newSessionDesign()) tabs().setActive("preview")
      },
    ),
  )
  ...
  createEffect(() => {
    if (params.id && timeline.loadFailed()) {
      showToast({
        variant: "error",
        title: language.t("common.requestFailed"),
        description: language.t("notification.session.error.fallbackDescription"),
      })
    }
  })
  ...
  const reviewReady = createMemo(() => {
    if (reviewMode() === "git" || reviewMode() === "branch") return !vcsQuery.isLoading
    return true
  })
  createEffect(() => {
    const msgsNotReady = !messagesReady()
    const reviewNotReady = !reviewReady()
    const promptNotReady = !prompt.ready()
    const rendering = store.deferRender
    global.sessionLoading.set(!!params.id && (msgsNotReady || reviewNotReady || promptNotReady || rendering))
  })
  ...
  const reviewPanelV2Props = () => ({
    ...
    get sidebarToggle() {
      return (
        <button
          type="button"
          class="hover:bg-v2-background-bg-layer-01 text-white rounded cursor-pointer flex items-center justify-center rounded-sm w-7 h-7 px-5"
          onClick={() => reviewV2State.toggleSidebar()}
          aria-label={language.t("command.fileTree.toggle")}
          aria-expanded={reviewV2State.sidebarOpened()}
        >
          <Icon name="work-tree" size="small" />
        </button>
      )
    },
  ...
  const sessionPanelContent = () => (
    <>
      {messagesReady() ? null : sessionSync() ?? ""}
      ...
      <div class="flex-1 min-h-0 overflow-hidden">
        <Switch>
          ...
          <Match when={params.id && visibleUserMessages()?.length > 0}>
  ...
  return (
    <SessionRouteFrame>
      <Show when={decidingPanelState()}>
        <div class="absolute inset-0 z-[100] bg-v2-background-bg-base flex items-center justify-center">
      ...
      <div
        ...
        classList={{
          "gap-1 p-1": settings.general.newLayoutDesigns() && !sessionPanelCollapsed(),
        }}
      >
        ...
        >
          ...
          <Show when={desktopSessionResizeOpen() && !sessionPanelCollapsed()}>
          ...
          <Show when={isDesktop() ? desktopV2PanelLayout().visible : terminalOpen()}>
            <div class="min-w-0 h-full flex flex-1 flex-col">
              <Show when={isDesktop() && (desktopV2ReviewOpen() || desktopFileTreeOpen())}>
                <div class="min-h-0 flex-1 flex gap-1">
                  <Show when={desktopV2ReviewOpen()}>
                      ...
                      <SessionSidePanel
                        ...
                        panelSizeToggle={
                          isDesktop() && (desktopV2ReviewOpen() || desktopFileTreeOpen()) ? (
                            <button
                              type="button"
                              class="flex items-center justify-center w-7 h-7 rounded-md cursor-pointer hover:bg-surface-base-hover transition-colors"
                              onClick={() => setSessionPanelCollapsed((prev) => !prev)}
                              title={
                                sessionPanelCollapsed()
                                  ? language.t("session.panel.restoreSplitView")
                                  : language.t("session.panel.collapseToReview")
                              }
                            >
                              <Icon
                                name={sessionPanelCollapsed() ? "sidebar-active" : "sidebar"}
                                size="small"
                        ...
                        fullScreen={sessionPanelCollapsed()}
                        onPreviewReady={setPreviewReady}
                        onActiveTabChange={setActiveSidePanelTab}
                        questionRequest={composer.questionRequest()}
                        onQuestionDone={() => {}}
                        agentName={agentModelName()}
                        hideFileTree={desktopFileTreeOpen()}
                      />
                    </div>
                  </Show>
                  <Show when={desktopFileTreeOpen()}>
                    <FileTreePanel diffs={reviewDiffs} size={size} />
                  </Show>
                </div>
              </Show>
