...
describe("server session", () => {
  ...
  test("clears loading after a failed initial load so a later sync retries", async () => {
    let calls = 0
    const client = {
      session: {
        get: async () => ({ data: session("child", "root") }),
        messages: async () => {
          ...
          return response([{ info: userMessage("message"), parts: [] }])
        },
      },
    } as unknown as OpencodeClient
    const store = createServerSession(client)

    await expect(store.sync("child")).rejects.toThrow("boom")
    expect(store.history.loading("child")).toBe(false)

    await store.sync("child")
    expect(store.data.message.child).toEqual([userMessage("message")])
  })
