...
export const { use: useSettings, provider: SettingsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    createEffect(() => {
      if (!ready()) return
      const enabled = store.general?.pinchZoom ?? defaultSettings.general.pinchZoom
      import("@/utils/browser-zoom").then(({ setPinchZoomEnabled }) => {
    ...
    return {
      ...
      general: {
        ...
        pinchZoom: withFallback(() => store.general?.pinchZoom, defaultSettings.general.pinchZoom),
        setPinchZoom(value: boolean) {
          setStore("general", "pinchZoom", value)
        },
        newLayoutDesigns,
