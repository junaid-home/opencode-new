...
function isContextGroupTool(part: PartType): part is ToolPart {
  ...
  if (part.tool === "read") {
    ...
    if (typeof input.filePath === "string" && mediaSourceFromPath(input.filePath)) return false
  }
  return true
}
...
const READ_IMAGE_EXTENSIONS = new Set(["png", "jpg", "jpeg", "gif", "webp", "avif", "bmp", "svg", "ico", "heic"])
const READ_VIDEO_EXTENSIONS = new Set(["mp4", "webm", "ogg", "ogv", "mov", "m4v"])

// Mirrors the server's /file/image and /file/video routes so media read into
// context stays visible even when the model does not embed it in its reply.
function mediaSourceFromPath(
  filePath: string | undefined,
): { src: string; path: string; kind: "image" | "video" } | undefined {
  if (!filePath) return
  const ext = filePath.split(".").pop()?.toLowerCase() ?? ""
  if (READ_IMAGE_EXTENSIONS.has(ext)) {
    return { src: `/file/image?path=${encodeURIComponent(filePath)}`, path: filePath, kind: "image" }
  }
  if (READ_VIDEO_EXTENSIONS.has(ext)) {
    return { src: `/file/video?path=${encodeURIComponent(filePath)}`, path: filePath, kind: "video" }
  }
  return
}
...
// Renders media attachments returned by a tool (webfetch, MCP servers, custom
// tools) as thumbnails below the tool call, with click-to-fullscreen preview.
function ToolMediaAttachments(props: { attachments?: Array<FilePart> }) {
  const i18n = useI18n()
  const dialog = useDialog()
  const media = createMemo(() =>
    (props.attachments ?? []).filter((file) => file.mime.startsWith("image/") || file.mime.startsWith("video/")),
  )
  const openMedia = (file: FilePart) => {
    dialog.show(() => <ImagePreview src={file.url} alt={file.filename ?? file.mime} mime={file.mime} />)
  }
  return (
    <Show when={media().length > 0}>
      <div data-component="read-tool-images">
        <For each={media()}>
          {(file) => (
            <div data-component="read-tool-image">
              {file.mime.startsWith("video/") ? (
                <video
                  src={file.url}
                  title={i18n.t("ui.imagePreview.alt")}
                  muted
                  preload="metadata"
              ...
              <button
                type="button"
                data-slot="read-tool-image-expand"
                aria-label={i18n.t("ui.imagePreview.alt")}
                title={i18n.t("ui.imagePreview.alt")}
                onClick={() => openMedia(file)}
              >
...
PART_MAPPING["tool"] = function ToolPartDisplay(props) {
  ...
  const completedAttachments = createMemo<Array<FilePart> | undefined>(() => {
    const state = part().state
    return state.status === "completed" ? state.attachments : undefined
  })

  // The read renderer already displays media with a recognized file extension
  // from its local path, so the generic attachment renderer would duplicate it.
  // Files with unrecognized extensions (e.g. image.bin) fall through to the
  // generic renderer via their attachment.
  const readMediaFromPath = createMemo(() =>
    part().tool === "read" ? mediaSourceFromPath(input().filePath as string | undefined) : undefined,
  )
  ...
  return (
    <Show when={!hideQuestion()}>
      <div data-component="tool-part-wrapper" data-timeline-part-id={part().id}>
        ...
        <Show when={!readMediaFromPath()}>
          <ToolMediaAttachments attachments={completedAttachments()} />
        </Show>
      </div>
    </Show>
  )
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    const media = createMemo(() => mediaSourceFromPath(props.input.filePath))
    ...
    const openImage = (src: string, path: string, mime?: string) => {
      dialog.show(() => <ImagePreview src={src} alt={path} mime={mime} />)
    }
    return (
      <>
        <BasicTool
          {...props}
          icon="glasses"
          defaultOpen={props.defaultOpen ?? !!media()}
          trigger={{
            title: i18n.t("ui.tool.read"),
              ...
              <div data-component="read-tool-image">
                {value().kind === "video" ? (
                  <video
                    ...
                    muted
                    preload="metadata"
                    onClick={() => openImage(value().src, value().path, "video/mp4")}
                    onError={(event) => event.currentTarget.remove()}
                  />
                ) : (
                  <img
                    ...
                    alt={value().path}
                    title={i18n.t("ui.imagePreview.alt")}
                    onClick={() => openImage(value().src, value().path)}
                    onError={(event) => event.currentTarget.remove()}
                  />
                )}
                <button
                  ...
                  title={i18n.t("ui.imagePreview.alt")}
                  onClick={() =>
                    openImage(value().src, value().path, value().kind === "video" ? "video/mp4" : undefined)
                  }
                >
                  <Icon name="expand" size="small" />
                </button>
        ...
        <Show when={loaded().length > 0 && !media()}>
          <For each={loaded()}>
            {(filepath) => (
