import { isImageUrl, isVideoUrl, localImageUrl, localVideoUrl, useMarked } from "@opencode-ai/ui/context/marked"
...
function markCodeLinks(root: HTMLDivElement) {
  ...
  for (const code of codeNodes) {
    ...
    if (isImageUrl(href)) {
      ...
      img.src = localImageUrl(href)
      img.alt = href
      img.setAttribute("data-slot", "markdown-image")
      const target = parentLink ?? code
    ...
    if (isVideoUrl(href)) {
      ...
      video.src = localVideoUrl(href)
      video.controls = true
      video.preload = "metadata"
      video.setAttribute("data-slot", "markdown-video")
      const target = parentLink ?? code
...
function probeMarkdownMedia(root: HTMLDivElement) {
  const images = Array.from(root.querySelectorAll<HTMLImageElement>('img[data-slot="markdown-image"]'))
  ...
  const videos = Array.from(root.querySelectorAll<HTMLVideoElement>('video[data-slot="markdown-video"]'))
...
function setupCodeCopy(
  ...
  onImageClick?: (src: string, alt: string | undefined) => void,
) {
  ...
  const handleClick = async (event: MouseEvent) => {
    ...
    const image = target.closest('img[data-slot="markdown-image"]')
    if (image instanceof HTMLImageElement) {
      event.preventDefault()
      onImageClick?.(image.src, image.getAttribute("alt") ?? undefined)
      return
    }
...
export function Markdown(
  ...
  createEffect(() => {
    ...
    const openImage = (src: string, alt: string | undefined) => {
